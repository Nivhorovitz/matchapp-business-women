...
function renderTags(){const row=$('quickTags');if(!row)return;const set=state.currentType==='practice'?['שותפת תרגול','Handle price objections','Better discovery questions','Closing conversations','Demo practice']:state.currentType==='accountability'?['שותפת אחריות','Consistent follow-up','Pipeline review','CRM habits','Follow-up systems']:['מנטורית','Pricing confidence','Founder-led sales','Clearer offer','Premium package selling'];row.innerHTML=set.map(t=>`<button class="tag ${state.selectedTags.includes(t)?'selected':''}" data-tag="${esc(t)}">${esc(t)}</button>`).join('');row.querySelectorAll('.tag').forEach(b=>b.onclick=()=>{const t=b.dataset.tag;state.selectedTags=state.selectedTags.includes(t)?state.selectedTags.filter(x=>x!==t):[...state.selectedTags,t];renderTags()})}
